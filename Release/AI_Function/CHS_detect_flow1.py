...
class CHS_Detect():
    def __init__(self, CHD_Name,image_path, User_ID):
        self.image_path = image_path
        self.CHD_Name = os.path.splitext(CHD_Name)[0]
        self.CHS_dir = os.path.join("CHS", self.CHD_Name)
        self.CHS_save_dir = os.path.join("CHS_Detect", self.CHD_Name)
    ...
    def main(self):
        ...
        clear_and_create_dir(self.CHS_save_dir)  # create folder for save correct CHS
        i = 1
        # input image from uploads
        for file_path in self.image_path:
            image_basename = os.path.basename(file_path)
            new_image_path = os.path.join(self.CHS_dir, image_basename)
            shutil.copy(file_path, new_image_path)
            print(f"{i}:move {file_path} to {self.CHS_dir}\n")
            i+=1 
...
def main(CHD_Name, image_path, User_ID):
    CHS_detect = CHS_Detect(CHD_Name, image_path, User_ID)
    CHS_save_dir = CHS_detect.main()
