...
def generate_view(source_imgs, elevation, azimuths, camera_distance, num_inference_steps):
    ...
    # Prepare input data:
    images = [img.resize((256, 256)).convert("RGB") for img in source_imgs]
...
def main(img_dir):
    source_image_dir = img_dir  # Replace with your directory
    elevation = 0.0
    azimuths = [-45.0, -30.0, -15.0, 15.0, 30.0, 45.0]
    camera_distance = 1.2
    num_inference_steps = 40
    # Load all images from the specified directory
    source_image_paths = [os.path.join(source_image_dir, fname) for fname in os.listdir(source_image_dir) if fname.lower().endswith(('png', 'jpg', 'jpeg'))]
    source_imgs = [Image.open(path) for path in source_image_paths]

    output_imgs = generate_view(source_imgs, elevation, azimuths, camera_distance, num_inference_steps)

    for i, output_img in enumerate(output_imgs):
        output_path = os.path.join(source_image_dir, f"image_{i}.png")
        output_img.save(output_path)
        print(f"Output image saved as {output_path}")

# if __name__ == "__main__":
#     # Example usage:
#     # source_image_dir = "CHD_Images\Anime002_Images"  # Replace with your directory
#     # elevation = 0.0
#     # azimuths = [-45.0, -30.0, -15.0, 15.0, 30.0, 45.0]
#     # camera_distance = 1.2
#     # num_inference_steps = 40

#     main(source_image_dir, elevation, azimuths, camera_distance, num_inference_steps)

#@title Alternative Inferencing: Gradio Demo
# import gradio as gr
# import torch
# import diffusers
# from diffusers import DiffusionPipeline
